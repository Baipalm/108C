# streamlit_nmf_costs.py (Real-Time Version)
import numpy as np
...
# Cache input-dependent results to avoid recomputation on each change
@st.cache_data(show_spinner=False)
def generate_matrices(matrix_rows, matrix_cols, rank, noise_level):
    np.random.seed(42)
...
@st.cache_data(show_spinner=False)
def run_nmf(V, rank, beta_loss, solver, max_iter):
...
V_clean, V_noisy = generate_matrices(matrix_rows, matrix_cols, rank, noise_level)

W_fro_clean, H_fro_clean, time_fro_clean, err_fro_clean = run_nmf(V_clean, rank, 'frobenius', 'cd', max_iter)
W_kl_clean, H_kl_clean, time_kl_clean, err_kl_clean = run_nmf(V_clean, rank, 'kullback-leibler', 'mu', max_iter)
W_fro_noisy, H_fro_noisy, time_fro_noisy, err_fro_noisy = run_nmf(V_noisy, rank, 'frobenius', 'cd', max_iter)
W_kl_noisy, H_kl_noisy, time_kl_noisy, err_kl_noisy = run_nmf(V_noisy, rank, 'kullback-leibler', 'mu', max_iter)
